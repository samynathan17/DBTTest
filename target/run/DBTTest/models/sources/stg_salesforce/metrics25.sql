  ...
  create or replace  view DATAFLOTEST_DATABASE.dbt_test_staging.metrics25  as (
    with date_range as 
(
...
select * from filtered
 ...
 timeframe as (
  ...
  on TimeFrameID = date_range.date_day
    ...
    SELECT
        ...
        '25'  as METRIC_ID,
        '1'  AS METRIC_CATEGORY_ID,
        'D' as TimeFrame_Type,
        opportunity.opp_stage_name as Segment_name,
 ...
 AS DW_INS_UPD_DTS
     from opportunity 
        join timeframe 
        on nvl(opportunity.OPP_LAST_UPDATED_DT, opportunity.OPP_INITIAL_CREATE_DT) between timeframe.DAY_START and timeframe.DAY_END
        join date_range
        on TimeFrameID = date_range.date_day
        group by
        Report_Dt,
 ...
 AS DW_INS_UPD_DTS
     from opportunity 
        join timeframe 
        on nvl(opportunity.OPP_LAST_UPDATED_DT, opportunity.OPP_INITIAL_CREATE_DT) between timeframe.WEEK_START and timeframe.WEEK_END
 ...
 AS DW_INS_UPD_DTS
     from opportunity 
        join timeframe 
        on nvl(opportunity.OPP_LAST_UPDATED_DT, opportunity.OPP_INITIAL_CREATE_DT) between timeframe.MONTH_START and timeframe.MONTH_END
        and cast( OPP_INITIAL_CREATE_DT as date) <= TimeFrameID
        join date_range
        on TimeFrameID = date_range.date_day
        group by
 ...
 AS DW_INS_UPD_DTS
     from opportunity 
        join timeframe 
        on nvl(opportunity.OPP_LAST_UPDATED_DT, opportunity.OPP_INITIAL_CREATE_DT) between timeframe.QUARTER_START and timeframe.QUARTER_END
        and cast( OPP_INITIAL_CREATE_DT as date) <= TimeFrameID
        join date_range
        on TimeFrameID = date_range.date_day
        group by
 ...
 AS DW_INS_UPD_DTS
     from opportunity 
        join timeframe 
        on nvl(opportunity.OPP_LAST_UPDATED_DT, opportunity.OPP_INITIAL_CREATE_DT) between timeframe.YEAR_START and timeframe.YEAR_END
        and cast( OPP_INITIAL_CREATE_DT as date) <= TimeFrameID
        join date_range
        on TimeFrameID = date_range.date_day
        group by
